    ...
    try {
      ...
      if (supportedDomains.includes(host)) {
        ...
        if (result.includes('href="/view-3d')) {
          ...
          output = `3D Model: <a target="_blank" href="https://${host}/view-3d${model_url}">https://${host}/view-3d${model_url}</a> (The zip file with the model can be found in the network tab in Developer tools)`;
        } else {
      ...
      output = `Error retrieving the URL: ${error.message}`;
    }
  }
